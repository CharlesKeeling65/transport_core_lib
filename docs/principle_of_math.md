...
#### 7.2.1 边界安全性：外扩搜索逻辑 (Halo/Padding)
为了消除跨块连接丢失风险（漏跨块风险），算法在分块处理时采用了**非对称过滤策略**：
-   **供应点 (Supply)**：严格限制在当前网格块边界 `[r0*grid_size, r1*grid_size)` 内。这确保了每条边只会被计算一次，避免重复。
-   **需求点 (Demand)**：搜索范围向四周外扩 `distance` 像素，即 `[r0*grid_size - distance, r1*grid_size + distance)`。
-   **原理证明**：由于供应点到需求点的最大距离为 `distance`，位于块边缘的供应点 $s$ 可能连接到相邻块内的需求点 $d$。通过外扩 `distance` 范围，我们保证了所有潜在的 $d$ 都在当前处理块的视野内。
